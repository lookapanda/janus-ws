...
> extends EventEmitter {
  ...
  async addPlugin<T extends IJanusWsPlugin<TSocket>>(plugin: T) {
    ...
    const json = await this.transaction<IJanusWsSuccessMessage, IAttachPayload>(
      'attach',
      'success',
      request
    );
  ...
  async removePlugin<T extends IJanusWsPlugin<TSocket>>(plugin: T) {
    const json = await this.transaction<IJanusWsSuccessMessage, IDetachPayload>(
      'detach',
      'success',
      {
  ...
  >(
    ...
    return new Promise((resolve, reject) => {
      ...
      const request = {
        janus: type,
        session_id: (payload && Number(payload.session_id)) || this.sessionId,
  ...
  private async keepAlive(isScheduled?: boolean) {
    if (!this.socket || !this.isConnected() || !this.isSessionEstablished()) {
      return;
    }
  ...
  private handleReconnect = () => {
    ...
    if (
      this.isConnecting ||
      (this.config.createSessionOnConnect && this.isSessionEstablished()) ||
  ...
  private onMessage = (event: MessageEvent) => {
    try {
      ...
      switch (janus) {
        ...
        case 'ack': {
          const transaction = this.getTransaction(data.transaction, janus);
        ...
        case 'success': {
          ...
          if (!pluginHandle) {
            debug('This handle is not attached to this session', message);
            return;
          }
        ...
        case 'slowlink': {
          const { uplink, nacks } = message as IJanusWsSlowlinkMessage;
          debug(
        ...
        case 'error': {
          ...
          if (error && error.code && !ignoredErrorCodes.includes(error.code)) {
            debug('Janus error response', message);
          }

          const transaction = this.getTransaction(message.transaction);
        ...
        case 'event': {
          ...
          const { plugindata, error_code } = message as IJanusWsEventMessage<
            any
          >;
